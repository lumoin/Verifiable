...
namespace Verifiable.Core.Cryptography
{
    ...
    {
        //The definitions are at https://w3c-ccg.github.io/did-method-key/#x2048-bit-modulus-public-exponent-65537.
        ...
        /// <summary>
        /// A constant for ASN.1 DER encoded integers (or bitstrings) that if the high byte is set
        /// </summary>
        ...
        public static byte[] Encode(ReadOnlySpan<byte> rsaModulusBytes)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(rsaModulusBytes), $"Length must be {Rsa2048ModulusLength} or {Rsa4096ModulusLength}.");
            }

            //Only RSA 2048 and 4096 byte lenghts are supported. So it is sufficient to check
            //against either one when pre-condition on entry ensures the incomding buffer is either one.
            var prefix = rsaModulusBytes.Length == Rsa2048ModulusLength ? Rsa2048Prefix : Rsa4096Prefix;
            ...
            int arrayLength = prefix.Length + 1 + rsaModulusBytes.Length + RsaSuffix.Length;
            ...
            //one is subtracted from the length. But then also by ASN.1 DER encoding
        ...
        public static byte[] Decode(ReadOnlySpan<byte> encodedRsaModulusBytes)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(encodedRsaModulusBytes), $"Length must be {Rsa2048DerEncodedBytesLength} or {Rsa4096DerEncodedBytesLength}.");
            }

            //The ASN.1 DER prolog is not needed, so the index is adjusted to bypass it.
            //The prefix length is the same for both 2048 and 4096 bit RSA keys and they
            //are the only supported ones. The DER encoded RSA public modulus length is checked on pre-conditions
            //so the length can be relied on. Since this is a zero based index and not length, -1 is added.
            int envelopPrologIndex = Rsa2048Prefix.Length + 1;

            //If the byte following DER envelope is 0x00 (and the one following less than 128 DEC or 0x80),
            //it means it's a padding byte according to DER encoding rules.            
            //Since the modulus values are exponents of two, so the MSB should be always set.
            //
            //By the ASN.1 DER encoding rules a 0x00 byte is inserted so that DER decoder does not misinterpret
            //an unsigned big endian number (or byte sequence) as signed integer. This too can be bypassed.
